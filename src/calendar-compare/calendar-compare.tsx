...
export function CalendarCompare() {
    ...
    const [timezonesPlus, setTimezones] = useState<(MomentZone | null)[]>([null]);
    const [numberOfTimezones, setNumberOfTimezones] = useState<number>(1)
 
    const [meetingLength, setMeetingLength] = useState(30);
    const [meetingTime, setMeetingTime] = useState<Moment | null>();
    ...
    const timezoneFields: any[] = []
    for(let index = 0; index < numberOfTimezones; index++) {
        timezoneFields.push(
            <div key={index}>
                <Autocomplete
                    title={`Foreign Timezone ${index + 1}`}
                    setValue={(value: string) => addTimezone(moment.tz.zone(value), index)}/>
            </div>
        )
    }

    const addTimezoneField = () => {
        if (numberOfTimezones >= 4) {
            return
        }

        setTimezones([...timezonesPlus, null])
        setNumberOfTimezones(numberOfTimezones + 1)
    }

    const addTimezone = (timezone: MomentZone | null, index: number) => {
        let newTimezones = []

        for (let count = 0; count < timezonesPlus.length; count++) {
            newTimezones.push(timezonesPlus[count])
    ...
    const displayTimezones = () => {
        return (
            <div>
                {
                    [firstTimezone, ...timezonesPlus].map((timezone) => {
                        return (
                            timezone &&
                            <div className="grid grid-cols-2">
                                <div className="text-sm md:text-base">{ timezone?.name }</div> 
                                {
                                    !(meetingLength < 15 || !meetingTime || !meetingTime.isValid()) ?
                                    <div className="text-sm md:text-base text-right">
                                        { moment(meetingTime).tz(timezone?.name ?? "")?.format("hh:mm a") } - { moment(meetingTime).tz(timezone?.name ?? "").add(meetingLength, "minutes")?.format("hh:mm a") }
                                    </div> :
                                    <div className="text-sm md:text-base text-right">No time selected</div>
                                }
                            </div>
                        )
                    })   
    ...
    const getTimezoneText = () => {
        ...
        let timezones = [firstTimezone, ...timezonesPlus];

        timezones.forEach((timezone) => {
            if (timezone) {
                timezoneText += timezone?.name + " " + moment(meetingTime).tz(timezone?.name ?? "")?.format("hh:mm a") + "-" + moment(meetingTime).tz(timezone?.name ?? "").add(meetingLength, "minutes")?.format("hh:mm a") + "\n"
            }
        })
    ...
    return(
        <div className="grid sm:grid-cols-1 md:grid-cols-6 md:mx-20">
            <div className="md:col-span-2 md:grid-rows-2 mx-8">
                <div>
                    ...
                    <div className="pt-6">
                        <div className="card w-full bg-base-200 text-nuetral-focus-content">
                            <div className="card-body">
                                <div className="grid grid-cols-2">
                                    <h2 className="card-title">Meeting Times</h2>
                                    <div className="text-right">
                                        <FontAwesomeIcon className="cursor-pointer hover:text-neutral" icon={faCopy} onClick={() => {navigator.clipboard.writeText(getTimezoneText())}} />
                                    </div>
                                </div>
                                { displayTimezones() }
                            </div>
            ...
            <div className="mx-4 md:col-span-4 md:mx-8 pt-6 md:pt-12">
                <Calendar
                    calendarId={1}
                    meeting={getMeeting(firstTimezone ?? null, meetingLength)}
                    setMeetingTime={(value: Moment) => setMeetingTime(value)}
                    date={date}
                    localTimezone={firstTimezone ?? null}
                    foreignTimezones={timezonesPlus} /> 
            </div>
        </div>
    )
}
